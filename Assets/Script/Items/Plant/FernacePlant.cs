...
public class FernacePlant : BasePlant, IRadiusPlaceable, IConsumable
{
    bool _withinRadius = false;
    TimedStatusEffect _consumableEffect = new TimedStatusEffect(PlayerConstants.StatusEffectType.Heating, 30.0f);
	...
	public void instantiateObject(Vector3Int gridPositionInt, GameObject selectedObj, TilemapRenderer curRenderer, Dictionary<Vector3Int, GameObject> objCoordinatesDict)
	{
        ...
        obj.GetComponent<FernacePlant>().StartCoroutine(PlaceAnimation());
	...
	{
		return type == InventoryItemType.PlaceableRadius || type == InventoryItemType.Expendable;
	}
    ...
    void OnTriggerExit2D(Collider2D other)
    {
        if (other.CompareTag("Player"))
        {
			Debug.Log("Left fernace plant radius.");
			_withinRadius = false;
			PlaceableAbility();
		}
    }
